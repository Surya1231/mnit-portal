...
import { cf_first_year , cf_second_year , cf_third_year } from '../../data/static/codeforces';
...
function getUserNameString(user){
    switch(user){
        ...
        default:
            return generateString(cf_first_year) + ";" + generateString(cf_second_year);
    }
}
...
export async function getCfLeaderboard(values){
    ...
    if(values.contest !== "overall"){
        const contestId = values.contest === "custom" ? values.customId : values.contest;
