...
const CfContest = ({data , contest}) => {
    return (
        ...
        <h3 className="text-center mb-4 text-primary">
            <a rel="noopener noreferrer" target="_blank" href={`https://codeforces.com/contest/${contest.id}`}> 
            ...
            <tbody>
                {data.map((item, index) => {
                    ...
                    return (
                        <tr key = {index}>
                            <td><b>{index+1}</b></td>
                            <td><a rel="noopener noreferrer" target="_blank" href={link}> {user_name}</a></td>
                            <td>{point}</td>
                            <td>{rank}</td>
                        </tr>
                    )
